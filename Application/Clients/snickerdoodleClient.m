classdef snickerdoodleClient
    ...
    methods (Static)
        ...
        function [img1, img2] = requestProcessedFrames(client, width, height)
            ...
            % Snickerdoodle is stupid and won't know how much
            % to send back unless we tell it. Stupid ass device
            % Also these names are awful. Oh well!
            floatsImageData = single([width, height, imageChannels, numberOfImages]);
            floatImageDataBytes = typecast(floatsImageData, 'uint8');
            write(client, floatImageDataBytes, 'uint8');
        ...
        function exampleConnection()
            ...
            % Create dummy images (RGBA, uint8)
            imageLeft     = uint8(randi([0,255], height, width, channels));
            baselineLeft  = uint8(randi([0,255], height, width, channels));
            imageRight    = uint8(randi([0,255], height, width, channels));
            baselineRight = uint8(randi([0,255], height, width, channels));

            % Send images to server
            snickerdoodleClient.sendImages(client, imageLeft, baselineLeft, imageRight, baselineRight);
            disp('[MATLAB CLIENT] Dummy images sent.');

            % Request processed frames
            [img1, img2] = snickerdoodleClient.requestProcessedFrames(client, width, height);
            disp('[MATLAB CLIENT] Processed frames received.');

            % Optionally display or save the images
            figure; imshow(img1); title('Processed Left');
            figure; imshow(img2); title('Processed Right');
